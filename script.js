...
let gameState = {
    coins: 0,
    bestScore: 0,
    unlocked: ['goofy'], // First free choice default safely handling fallback
    selected: 'goofy',
    isFirstLaunch: true
};
...
// Preloading Image Pools (Updated to target 'images/' folder)
const images = {};
...
window.onload = () => {
    ...
    if (gameState.isFirstLaunch) {
        // Allow choosing the first one completely free
...
function renderCarousel() {
    ...
    CREATORS.forEach((c, index) => {
        const isUnlocked = gameState.unlocked.includes(c.id);
        const isActive = index === currentCarouselIndex;
        const card = document.createElement('div');
        card.className = `character-card ${isActive ? 'active' : ''}`;
        
        // Updated to target 'images/' folder
        card.innerHTML = `
            <img src="images/${c.id}standing.png" alt="${c.name}">
            <div class="name">${c.name}</div>
            <div class="status">${isUnlocked ? (gameState.selected === c.id ? 'Selected' : 'Owned') : `🔒 ${c.cost} Coins`}</div>
        `;
        track.appendChild(card);
    });
    
    // Smooth centering offset
    ...
    if (gameState.unlocked.includes(activeChar.id)) {
        confirmBtn.innerText = "Select & Run";
...
function confirmSelection() {
    ...
    if (gameState.unlocked.includes(selectedChar.id)) {
        if(gameState.isFirstLaunch) {
            // Lock down remaining characters after free choice selection
...
function spawnObstacle() {
    ...
    obstacles.push({
        x: canvas.width + 100,
        y: 380 - type.height, // Adjusted alignment to ground
        width: type.width,
        height: type.height,
        id: type.id
    });
    
    // Scaling Dynamic Timing Gap Generator Rule Logic
    let nextSpawnTime = Math.random() * 1500 + 1000 / currentSpeedMultiplier;
    setTimeout(spawnObstacle, nextSpawnTime);
}

// The Core Game Engine Loop
function gameLoop() {
    if (!isPlaying) return;
    ...
    // 1. Moving Seamless Background Maps Tracker
    ...
    // Toggle Jump/Run Sprite Animations
    ...
    // 3. Render, Move & Check Collision of obstacles
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        // Inner Padding Safe Collision Boxes rule criteria logic check
        if (
        ...
        // Score tracking out-of-screen deletion
        if (obs.x + obs.width < player.x && !obs.passed) {
            obs.passed = true;
            score++;
            gameState.coins += 1; // 1 Coin per obstacle passed
            adjustDifficulty();
            updateHUD();
        }
...
function adjustDifficulty() {
    ...
    else currentSpeedMultiplier = 1 + (score * 0.1); // Gradual step
}
...
// Game Over Handler Sequence
function handleGameOver() {
    isPlaying = false;
    cancelAnimationFrame(animationFrameId);
    window.removeEventListener('keydown', handleInput);

    // Play Creator Distinct Sound clip (Assuming audio files are still in the root directory)
    let failAudio = new Audio(`${gameState.selected}.mp3`);
    failAudio.play().catch(e => console.log("Audio play blocked by browser rules context interaction or file missing"));
